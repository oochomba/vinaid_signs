    ...
    <div class="container-fluid services pt-0">
        <div class="container py-5">
            <div class="pb-1 wow fadeIn" data-wow-delay=".3s">
                <h2 class="text-primary">{{ $category->name }}</h5>
            </div>
            <div class="row">
                <div class="col-md-12">
                    ...
                    @if (!empty($subCategories->count()))
                        @foreach ($subCategories as $subCategory)
                            <h3 class="small-text my-anchor pt-2 pb-1">
                                {{ $loop->iteration . '. ' }} <a
                                    href="{{ $category->slug . '/' . $subCategory->slug }}">{{ $subCategory->name }}</a>
                            </h3>
                            @if (!empty($subCategory->description))
                                {!! $subCategory->description !!}
                            @endif

                            @if (!empty($subCategory->children->count()))
                                            ...
                                            <div class="col-lg-4">
                                                <article>
                                                    <div class="post-img">
                                                        ...
                                                        @endphp
                                                        <a href="{{ url($serviceImage->getImageInfo()) }}">
                                                            <img src="{{ url($serviceImage->getImageInfo()) }}"
                                                                alt="{{ $children->name }}" class="img-fluid">
                                                        </a>
                                                    </div>
                                                    <h5 class="pt-1">
                                                        <a
                                                            href="{{ url($category->slug . '/' . $subCategory->slug . '/' . $children->slug) }}">{{ $children->name }}</a>
                                                    </h5>
                                                    ...
                                                    <p class="my-anchor">
                                                        <a
                                                            href="{{ url($category->slug . '/' . $subCategory->slug . '/' . $children->slug) }}">
                                                            Read More...
                                                        </a>
                                                    </p>
                            ...
                            @else
                                ...
                                @if (!empty($subCategoryImages->count()))
                                            ...
                                            <div class="portfolio-item accessories women col-sm-6 col-md-4 col-lg-3">
                                                <div class="portfolio portfolio-overlay">
                                                    <figure class="portfolio-media">
                                                        <a href="{{ url($subCategoryImage->getImageInfo()) }}">
                                                            <img src="{{ url($subCategoryImage->getImageInfo()) }}"
                                                                alt="item">
                                                        </a>
                                                    </figure>
                    ...
                    @else
                        ...
                        @if (!empty($categoryImages->count()))
                            <div class="border-top mt-4">
                                <h3 class="title text-center big-text pt-2 pb-2">{{ $category->name }} Portfolio</h3>
                            </div>
                            <div class="portfolio-container" data-layout="fitRows" id="portfolio-2">
                                @foreach ($categoryImages as $categoryImage)
                                    @if (!empty($categoryImage->getImageInfo()))
                                        <div class="portfolio-item accessories women col-sm-6 col-md-4 col-lg-3">
                                            <div class="portfolio portfolio-overlay">
                                                <figure class="portfolio-media">
                                                    <a id="filters" href="{{ url($categoryImage->getImageInfo()) }}">
                                                        <img src="{{ url($categoryImage->getImageInfo()) }}"
                                                            alt="item">
                                                    </a>
                                                </figure>
                                                <div class="portfolio-content">
                                                    <h3 class="portfolio-title"><a
                                                            href="{{ $category->slug }}">{{ $categoryImage->name }}</a>
                                                    </h3>
                                                    <div class="portfolio-tags">
                                                        <a href="{{ $category->slug }}">{{ $category->name }}</a>
                                                    </div>
...
@section('scripts')
    <script src="{{ url('assets/front/assets/javascripts/jquery.waypoints.min.js') }}"></script>

    <script src="{{ url('assets/front/assets/javascripts/imagesloaded.pkgd.min.js') }}"></script>
    <script src="{{ url('assets/front/assets/javascripts/isotope.pkgd.min.js') }}"></script>
